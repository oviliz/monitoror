...
func TestLoadConfig(t *testing.T) {
	...
	// TEST
	LoadConfig(prefix, defaultVariant, &conf, defaultValue)
...
func TestLoadConfig_Panic(t *testing.T) {
	...
	assert.Panics(t, func() { LoadConfig("", "", nil, nil) })
	assert.Panics(t, func() { LoadConfig("", "", conf, &defaultValue) }) // Need pointer
	assert.Panics(t, func() { LoadConfig("", "", &conf, defaultValue) }) // Need pointer

	conf2 := make(map[int]TestStruct)
	assert.Panics(t, func() { LoadConfig("", "", &conf2, defaultValue) }) // Need *map[string] ...
}
