...
func LoadConfig(envPrefix, defaultVariant string, conf interface{}, defaultConf interface{}) {
	// Verify Params
	if reflect.ValueOf(conf).Kind() != reflect.Ptr {
		panic(fmt.Sprintf("wrong LoadConfig parameters: conf need to be a pointer of map[string]struct not a %s", reflect.ValueOf(conf).Kind()))
	}
	if reflect.ValueOf(conf).Elem().Kind() != reflect.Map || reflect.ValueOf(conf).Elem().Type().Key().Kind() != reflect.String {
		panic(fmt.Sprintf("wrong LoadConfig parameters: conf need to be a pointer of map[string]struct not a %s", reflect.ValueOf(conf).Elem().Type()))
	}
	if reflect.ValueOf(defaultConf).Kind() != reflect.Ptr {
		panic(fmt.Sprintf("wrong LoadConfig parameters: defaultConf need to be a pointer of struct not a %s", reflect.ValueOf(conf).Kind()))
	}
